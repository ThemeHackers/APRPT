...
/* s.l2cap_send_high_speed(data) method */
static PyObject *
sock_l2cap_send_high_speed(PySocketSockObject *s, PyObject *arg)
{
	Py_buffer buf;
	int n = 0;
	...
	/* Direct send without select, optimizing for speed */
	Py_BEGIN_ALLOW_THREADS
	n = send(s->sock_fd, buf.buf, buf.len, 0);
	Py_END_ALLOW_THREADS
...
PyDoc_STRVAR(l2cap_send_high_speed_doc,
"l2cap_send_high_speed(data) -> count\n\
\n\
Send L2CAP data with minimal overhead. No timeout checks.");
...
static PyMethodDef sock_methods[] = {
	...
	{"sendto",	(PyCFunction)sock_sendto, METH_VARARGS,
			sendto_doc},
	{"l2cap_send_high_speed",	(PyCFunction)sock_l2cap_send_high_speed, METH_O,
			l2cap_send_high_speed_doc},
...
/* hci_le_set_advertising_data(sock, data) */
static PyObject *
bt_hci_le_set_advertising_data(PyObject *self, PyObject *args)
{
	PySocketSockObject *socko = NULL;
	char *data = NULL;
	...
	if (!PyArg_ParseTuple(args, "Os#", &socko, &data, &data_len)) {
		return NULL;
	}

	if (data_len > 31) {
		PyErr_SetString(PyExc_ValueError, "Data too long (max 31 bytes)");
		return NULL;
	}

	memset(&cp, 0, sizeof(cp));
	cp.length = data_len;
	memcpy(cp.data, data, data_len);

	dd = socko->sock_fd;

	Py_BEGIN_ALLOW_THREADS
	err = hci_send_cmd(dd, OGF_LE_CTL, OCF_LE_SET_ADVERTISING_DATA, sizeof(cp), &cp);
	Py_END_ALLOW_THREADS
...
PyDoc_STRVAR(bt_hci_le_set_advertising_data_doc,
"hci_le_set_advertising_data(sock, data)\n\
\n\
Send LE Set Advertising Data command directly.");
...
static PyMethodDef bt_methods[] = {
    ...
    DECL_BT_METHOD( hci_le_set_advertising_data, METH_VARARGS ),
